...
milking_period_detailed <- function(df_fields, df_milk,  params, time, cleaning_time=NA){
  N_milk <- nrow(df_milk)
  ...
  for(i in 1:N_fields){
    ...
    while(sum(df_fields_temp[i,1:4])>0){
      ...
      g_events <- g_simulation_period_detailed(df_fields=df_fields_temp,  params=params, time=time, next_milking_time = time+params$milk_time_cow)
      ...
      milk_log_temp <- df_milk
      milk_log_temp$time <- time
    ...
    g_events <- g_simulation_period_detailed(df_fields=df_fields,  params=params, time=time, next_milking_time = time+params$milk_time_field)
  ...
  post_milking_outcomes <- list(df_fields, df_milk, time, event_log, milk_log)
