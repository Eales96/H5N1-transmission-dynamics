...
simulation_reactive <- function(df_fields, df_milk,  params, time=0, n_days=100,
                                milking_time=6/24, milking_frequency=2,
                                cleaning_time=NA,
                                reaction_threshold=10000,
                                N_fields=5){
  reacted <- "no"
  inc <- data.frame()
  ...
  while(while_con){
    ...
    if(reacted=="no"){
      if(df_fields$N_E+df_fields$N_I+df_fields$N_R>=reaction_threshold){
        
        df_fields <- reactive_cohort(df_fields=df_fields, N_fields=N_fields)
        reacted <- "yes"
      }
    }
